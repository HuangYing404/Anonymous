...
parser.add_argument('--batch-size', type=int, default="4",
                    help='the batch size')
parser.add_argument('--presence-obj', action='store_true', default=False,
...
parser.add_argument('--client', type=int, default="4",
...
if args.graph_prior:
    ...
    print('regularization coefficient: ' + str(args.w1) + ' ' + str(args.w2) + ' ' + str(args.w3))
if args.tag_loss:
    print('Using TAG Loss with coefficient ' + str(args.wt))
# print('#########################################')


dataset = MimicDataset(train=False)
...
for i, data in enumerate(train_loader):
    ...
    gt_diagnose_multihot, gt_procedure_multihot, gt_medicine_multihot = gt_diagnose_multihot.to(
        device), gt_procedure_multihot.to(device), gt_medicine_multihot.to(device)
    ...
    if args.model == 'transformer':
        net = TransformerEncoder_mimic(input_size=DIAG_NUM + PROC_NUM, embedding_size=EMBEDDING_SIZE,
                                       output_size=MED_NUM).to(device)
    ...
    diagnose_auc = roc_auc_score(gt_diagnose_multihot.cpu().numpy(), dummy_diag_pred.detach().cpu().numpy(),
                                 average='micro')
    procedure_auc = roc_auc_score(gt_procedure_multihot.cpu().numpy(), dummy_proc_pred.detach().cpu().numpy(),
                                  average='micro')
    medicine_auc = roc_auc_score(gt_medicine_multihot.cpu().numpy(), dummy_med_pred.detach().cpu().numpy(),
                                 average='micro')
    ...
    diag_auc_list.append(diagnose_auc)
    proc_auc_list.append(procedure_auc)
    med_auc_list.append(medicine_auc)
